...
const mockConfig: WorldConfig = {
  ...
  time: { elasticHeartbeat: false, maxHeartbeatWaitMs: 5000, tickDurationMs: 1 },
...
test("Orchestrator: Full Integration test (5 agents, 500 ticks)", async () => {
  ...
  expect(clock.getTick()).toBe(500);

  const logs = db.getAuditLogs("main");
  expect(logs.length).toBeGreaterThan(0);

  const verifyResult = MerkleLogger.verifyChain("main");
